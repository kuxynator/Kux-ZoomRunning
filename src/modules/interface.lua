...
modules.interface = {
	...
	onZoomFactorChanged_raise = function (player, zoomFactor, renderMode)		
		...
		--log(eventArgs.playerIndex..", "..eventArgs.zoomFactor..", "..eventArgs.renderMode)
		Running.onZoomFactorChanged(eventArgs)
		local count = 0
		for interfaceName,functionName in pairs(callbacks.onZoomFactorChanged) do
			count = count + 1
			if functionName == nil then goto next end
			--log("["..count.."] remote.call "..interfaceName..", "..functionName..", "..eventArgs.playerIndex..", "..eventArgs.zoomFactor..", "..eventArgs.renderMode)
			local f = function() remote.call(interfaceName, functionName, eventArgs) end
			local status, ex = pcall(f)
			if not status then
				callbacks.onZoomFactorChanged[interfaceName] = nil
				print("Kux-ZoomRunning: ERROR callback failed. Registration removed. interface: '"..interfaceName.."', function: '"..functionName.."'\nException: "..ex )
				game.print("The mod caused a error.\nPlease report this error to the mod author (not Kux-ZoomRunning).\n\n"..ex ,{1,0.5,0.5})
			end
			::next::
		end
	end,
